...
title: Clean Architectureにおいて、バリデーションはどこでやるべきか / アプリケーションロジックとドメインロジックの違い
...
「ユーザーは JSON 形式でリクエストできる」「ユーザーは任意の文章を投稿できる」といった仕様があればまた、その裏返しとして「JSON 以外の形式は受け付けない」「本文が空白の場合は投稿できない」といったバリデーションが必要というだけの話である。
...
`ECサイトにおいて、いたずらでの大量発注による攻撃を防ぐため、注文できる上限個数を99個に制限する。`
...
`ECサイトにおいて、1人あたりの購入上限個数が決まっている限定セールを行う際、一人が注文できる上限個数は3個に制限する。`
...
たとえば文章投稿サービスにおける、「悪意ある人物による攻撃を考慮して、一応の上限として書き込みの際の文字数は10000文字までとしておく」というバリデーションについて考える。  
...
最初のうちは問題にならないかもしれないが、たとえばもしあとで、「やっぱり5000文字に制限をきつくしよう」といった変更が入った場合について考える。
...
アプリケーション層に書けば、バリデーションが走るのは投稿のユースケースのみに限るようにすることができ、既存の投稿には一切影響を与えずに新規の投稿についてのみ文字数制限を厳しくすることができる。
